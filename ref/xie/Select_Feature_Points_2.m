function [Points_l,num,CornerPoint,CornerPoint_int,CornerImage]=Select_Feature_Points_2()
...
[filename,pathname]=uigetfile('*.pcd','选择数据文件');
...
%这个地方用我自己写的代码和ros采的数据坐标有点差别，所以有这部分
% aa = loc(:,2);
% loc(:,2)=loc(:,1);
% loc(:,1)=-aa;
% loc(:,4)=double(ptCloud.Intensity);
...
for j =1 :1:2
        ...
        if (EagePointImage_out(x(i),y(i)) ~=1)  %如果未选中端点
            [lx,ly]=find (EagePointImage_out(x(i)-wid:x(i)+wid,y(i)-wid:y(i)+wid)==1);   %找到wid近邻的端点位置
                ...
                if (j==1) 
                    x(i) = x(i)-wid-1+lx(1);
                    y(i) = y(i)-wid-1+ly(1);
                else       %当选y轴时，如果未选到端点，选取最右边的点         
                    x(i) = x(i)-wid-1+lx(end);
                    y(i) = y(i)-wid-1+ly(end);
                end
 ...
 [Corner, Ptr_2,corner_point]=CalibPatterFitting_2(T,f,size_u,L_l{1},L_l{2},resolution);


num=input('Please Input the number of points:');
Points_l = zeros(num,3);
...
for j = 1:1:num   
    ...
    y(j) = int16(y(j));
        ...
        if (Corner(x(j),y(j)) ~=1)  %如果未选中端点
            [lx,ly]=find (Corner(x(j)-wid:x(j)+wid,y(j)-wid:y(j)+wid)==1);   %找到wid近邻的端点位置
            x(j) = x(j)-wid-1+lx(1);
            y(j) = y(j)-wid-1+ly(1);
        end
        ...
        Points_l(j,:) = corner_point{Ptr_2{x(j),y(j)}(1),Ptr_2{x(j),y(j)}(2)};
