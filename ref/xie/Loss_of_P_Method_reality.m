...
function loss = Loss_of_P_Method_reality(Tht,Ins_para_P,Points_c_final_undistortion,Points_l_final,resolution)
 num = size(Points_c_final_undistortion,1);
 R = rodrigues(Tht(1:3));
 T = Tht(4:6);
 RT_P = [R,T];
[Points_project,~] = project(RT_P*[Points_l_final';ones(1,num)],Ins_para_P);
