...
elseif ischar(images1)
    images = {images1, images2}; %#ok<EMCA>
else
...
validateattributes(showProgressBar, {'logical', 'numeric'},...
    {'scalar'}, mfilename, 'ShowProgressBar'); %#ok<EMCA>
tf = true;
...
validClasses = {'double', 'single', 'uint8', 'int16', 'uint16'}; %#ok<EMCA>
validateattributes(images, validClasses,...
    {'nonempty', 'real', 'nonsparse'},...
    mfilename, 'images'); %#ok<EMCA>
...
validateattributes(fileNames, {'cell'}, {'nonempty', 'vector'}, mfilename, ...
    'imageFileNames'); %#ok<EMCA>
for i = 1:numel(fileNames)
    checkFileName(fileNames{i}); %#ok<EMCA>
end

%--------------------------------------------------------------------------
function checkFileName(fileName)
validateattributes(fileName, {'char'}, {'nonempty'}, mfilename, ...
    'elements of imageFileNames'); %#ok<EMCA>
try %#ok<EMTC>
    imfinfo(fileName);
catch e
    throwAsCaller(e);
...
for i = 1:numImages
    ...
    im = imread(fileNames{i}); %#ok<EMCA>
    [boardPoints{i}, boardSizes(i,:)] = detectCheckerboardInOneImage(im);     %#ok<EMCA>
    if showProgressBar
        waitBar.update();
    end    
end
...
modeBoardId = max(modes{1}); %#ok<EMCA>
