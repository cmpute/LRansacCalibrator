...
classdef Checkerboard < handle
    ...
    methods
        ...
        %------------------------------------------------------------------
%         function orient(this, I)
%             if ~isinf(this.Energy)
%                 % orient the board so that the long side is the X-axis
%                 if size(this.BoardCoords, 1) < size(this.BoardCoords, 2)
%                     rot90(this, 1);
%                 end
%                 
%                 % try to orient the board so that (0,0) is on a black square
%                 if ~isUpperLeftBlack(this, I);
%                     rot90(this, 2);
%                 end
%                 
%                 % if both sides are odd or both sides are even, make sure
%                 % that (0,0) is at the upper-left corner.
%                 if ~xor(mod(size(this.BoardCoords, 1), 2) == 0,...
%                         mod(size(this.BoardCoords, 2), 2) == 0)
%                     if any(this.BoardCoords(1,1,:) > this.BoardCoords(end, end, :))
%                         rot90(this, 2);
%                     end
%                 end
%             end
